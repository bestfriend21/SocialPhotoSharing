...
- (NSArray *)aschoursInCurrentDayAsDatesCollection
{
...
- (NSArray *)hoursInCurrentDayHourAsDatesCollection
{
    NSInteger currenthour = [self hourOfDay];
...
- (NSUInteger)timeAsSecond {
    
    return [self hourOfDay] * 3600 + [self minuteOfHour] * 60 + [self secondOfMinute];
}

+ (NSUInteger)daysFromTimeInterval:(NSTimeInterval)interval {
    NSInteger ti = (NSInteger)interval;
    NSInteger days = (ti / ( 3600 * 24 ));
    return days;
}

+ (NSUInteger)hoursFromTimeInterval:(NSTimeInterval)interval {
    NSInteger ti = (NSInteger)interval;
    NSInteger hours = (ti / 3600);
    return hours;
}

+ (NSUInteger)minutesFromInterval:(NSTimeInterval)interval {
    NSInteger ti = (NSInteger)interval;
    NSInteger minutes = (ti / 60) % 60;
    return minutes;
}

+ (NSUInteger)secondsFromInterval:(NSTimeInterval)interval {
    NSInteger ti = (NSInteger)interval;
...
+ (NSString *)stringFromTimeInterval:(NSTimeInterval)interval {
    ...
    if ( hours > 0 )
        timeStr = [NSString stringWithFormat:@"%d hrs ", hours];

    timeStr = [timeStr stringByAppendingString:[NSString stringWithFormat:@"%d mins ", minutes]];
